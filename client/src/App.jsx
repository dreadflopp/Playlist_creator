...
function App() {
    ...
    const handleLogin = async () => {
        ...
        try {
            ...
            const loginUrl = forceLogout ? "http://localhost:3000/api/auth/login?force_logout=true" : "http://localhost:3000/api/auth/login";
    ...
    const handleSendMessage = async (message, isRetry = false) => {
        ...
        try {
            ...
            // - recentMessages for intent detection (last 4 messages including current one for context)
    ...
    const handleUpload = () => {
        ...
        const verifiedCount = currentPlaylist.songs.filter((s) => typeof s === "object" && s.verified).length;
        ...
        <div className="min-h-screen bg-[#121212]">
            <div className="container mx-auto px-4 py-8 max-w-6xl">
                <div className="flex items-center justify-center gap-4 mb-8">
                    <h1 className="text-4xl font-bold text-white">AI Playlist Generator</h1>
                    <button onClick={handleReset} className="px-4 py-2 bg-[#282828] text-white rounded-lg hover:bg-[#404040] transition-colors text-sm flex items-center gap-2" title="Reset chat and playlist">
                    ...
                    <div className="flex items-center justify-between text-xs text-[#b3b3b3]">
                        <div className="flex items-center gap-6">
                            <div>
                                ...
                                {lastRequestStats ? (
                                    <span className="text-white">
                                        {lastRequestStats.total_tokens.toLocaleString()} tokens ({lastRequestStats.prompt_tokens.toLocaleString()} + {lastRequestStats.completion_tokens.toLocaleString()}) • ${lastRequestStats.cost_usd.toFixed(6)} • {lastRequestStats.model}
                                    </span>
                                ) : (
                            ...
                            <div>
                                ...
                                <span className="text-white">
                                    {cumulativeStats.total_tokens.toLocaleString()} tokens ({cumulativeStats.total_prompt_tokens.toLocaleString()} + {cumulativeStats.total_completion_tokens.toLocaleString()}) • ${cumulativeStats.total_cost_usd.toFixed(6)}
                                </span>
                            </div>
                        </div>
                    </div>
            ...
            <PlaylistNameModal isOpen={showPlaylistNameModal} onClose={() => setShowPlaylistNameModal(false)} onConfirm={handlePlaylistNameConfirm} currentName={currentPlaylist?.name} />
        </div>
    );
}
