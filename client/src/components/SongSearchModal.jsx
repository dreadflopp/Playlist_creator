  ...
  useEffect(() => {
    if (isOpen && currentSong) {
      ...
      if (typeof currentSong === 'string') {
        ...
        const parts = currentSong.name.split(' - ')
        song = parts[0] || ''
        artist = parts.slice(1).join(' - ') || ''
      }
      ...
      if (song.trim() || artist.trim()) {
        ...
        const performInitialSearch = async () => {
          ...
          try {
            const searchData = {
              query: song.trim() && artist.trim() 
                ? `${song.trim()} ${artist.trim()}` 
                : song.trim() || artist.trim(),
        ...
        <div className="flex-1 overflow-y-auto">
          {isSearching ? (
            ...
            <div className="text-center py-8 text-[#b3b3b3]">
              {songName || artistName ? 'No results found' : 'Enter a song name or artist to search'}
            </div>
          ) : (
            <div className="space-y-2">
              {results.map((track) => (
                <div
                  ...
                  className="bg-[#282828] hover:bg-[#404040] rounded-lg p-3 cursor-pointer transition-colors flex items-center gap-3"
                >
                  {track.image && (
                    <img
                      src={track.image}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {track.album && (
                      <p className="text-[#727272] text-xs truncate">{track.album}</p>
                    )}
                  </div>
                  <svg
