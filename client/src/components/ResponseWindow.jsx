...
function ResponseWindow({ playlist, onUpdateSong, onRemoveSong, onAddSong }) {
  ...
  const currentSong = selectedSongIndex !== null && playlist 
    ? playlist.songs[selectedSongIndex] 
    : null
        ...
        <div className="flex-1 min-h-0 flex flex-col">
          {playlist ? (
            <div className="bg-[#121212] rounded-lg p-4 border border-[#282828] flex flex-col flex-1 min-h-0">
              ...
              <div className="space-y-1 flex-1 overflow-y-auto min-h-0">
                {playlist.songs.map((song, idx) => {
                  // Handle both string format (backwards compatibility) and object format
                  const songName = typeof song === 'string' ? song : song.name;
                  const isVerified = typeof song === 'object' ? song.verified : false;
                  const albumImage = typeof song === 'object' ? song.image : null;
                  
                  return (
                    <div
                      key={idx}
                      className="bg-[#181818] hover:bg-[#282828] rounded px-3 py-2 text-sm text-white transition-colors flex items-center gap-2 group"
                    >
                      {/* Album thumbnail */}
                      {albumImage ? (
                        <img
                      ...
                      <div className="flex items-center gap-1">
                        {isVerified ? (
                          ...
                          <svg 
                            className="w-4 h-4 text-[#727272] flex-shrink-0" 
                            fill="currentColor" 
                            viewBox="0 0 20 20"
                        ...
                        <button
                          ...
                          className="opacity-0 group-hover:opacity-100 transition-opacity text-[#ef4444] hover:text-[#f87171] p-1"
            ...
            <div className="bg-[#121212] rounded-lg p-4 border border-[#282828] flex-1 flex flex-col items-center justify-center min-h-0">
              <p className="text-sm text-[#b3b3b3] text-center mb-4">
                No playlist created yet. Chat with the AI to generate one or add songs manually!
              </p>
              <button
                onClick={handleAddSongClick}
